...
class CourseListController extends Controller
{
                    ...
                    foreach ($excel_data as $key => $data) {
                        ...
                        if ($excel_data[$key]->filter()->isNotEmpty()) {
                            ...
                            if (count($check_events) != 0) {
                                ...
                                $events_course->memo             = '';                   // 課程備註
                                $events_course->id_group         = strtotime($date) . $id_course;     // 群組ID
                                $events_course->unpublish        = 0;                    // 上架/下架/取消場次
                                $events_course->save();
                                $id_events = $events_course->id;
                            }
    ...
    public function delete(Request $request)
    {
        ...
        $events = EventsCourse::where('id_course', $course->id)
                            ->get();
        ...
        if (!empty($course)) {
            if ($course->type == 1) {
                ...
                //刪除報名表
                SalesRegistration::where('id_course', $course->id)->delete();
                ...
                //將報名表更新為為選場次
                Registration::where('id_course', $course->id)
                                        ->update([
                                            'id_course' => -99,
                                            'id_group' => -99,
                                            'id_events' => -99,
            ...
            //更新課程階級
            Course::where('id_type', $course->id)
                    ->update(['id_type' => null]);
