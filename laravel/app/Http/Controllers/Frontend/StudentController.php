...
class StudentController extends Controller
{
    ...
    public function coursedata(Request $request)
    {
        ...
        $datas = SalesRegistration::leftjoin('isms_status', 'isms_status.id', '=', 'sales_registration.id_status')
                            ...
                            ->selectRaw('(SELECT COUNT(b.id) FROM sales_registration b ) as count_sales')
                            ->selectRaw("(SELECT SUM(CASE WHEN c.id_status = '4' THEN 1 ELSE 0 END)  FROM sales_registration c) as count_sales_ok")
                            ->selectRaw(" (SELECT SUM(CASE WHEN b.id_status = '5' THEN 1 ELSE 0 END)  FROM sales_registration b) as count_sales_no")
